...
//					echo '<script>alert("YOU ARE ALREADY LOGGED IN")</script>';
					...
					{
						$ud=mysql_query("UPDATE logs set total_work='4', remarks='HALF DAY',stat1='0', tag='0' where emp_id='$id' and tag='1'");
...
if($timein=='00:00')
{
	if($time>$advance)
	{
		if($time<'23:59')
		{
				$insert=mysql_query("INSERT into logs (emp_id,dt,time_in,minlate,deduction,remarks,stat1,tag)
						VALUES('$id','$date','$time','0','0','ON TIME','0','1')");
		...
		{
		$insert=mysql_query("INSERT into logs (emp_id,dt,time_in,minlate,deduction,remarks,stat1,tag)
			VALUES('$id','$date','$time','0','0','LOG IN','0','1')");
		...
		elseif($time>$allowance)
		{
			...
			elseif($time<=$latetime)
			{
				...
				while($row=mysql_fetch_array($sql))
				{
					...
					$insert=mysql_query("INSERT into logs (emp_id,dt,time_in,minlate,deduction,remarks,stat1,tag)
						VALUES('$id','$date','$time','$ml','$total_deduc','$remarks','0','1')");
...
if($time>=$advance)
{
	if($time<=$allowance)
	{
		$insert=mysql_query("INSERT into logs (emp_id,dt,time_in,minlate,deduction,remarks,tag)
						VALUES('$id','$date','$time','0','0','ON TIME','1')");
...
elseif($time<$advance)
{
	if($time>$timeout)
	{
	echo "<script>alert('TOO EARLY TO LOG IN1')</script>";
	}
	elseif($time<$timeout)
	{
		echo "<script>alert('MORE THAN 30 MINUTES LATE')</script>";
