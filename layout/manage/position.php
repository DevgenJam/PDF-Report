...
$query = "SELECT * FROM position order by cid DESC";
    ...
    <td align="center"><?php echo $row['name']; ?></td>
      <td align="center"><a href="editpos.php?cid=<?php echo $row['cid']; ?>" >EDIT</a></td>
   </tr>
<?php } ?>
</div></div></tbody></table>
...
if(isset($_POST['sav']))
{
	$sql=mysql_query("SELECT * FROM position where name='$pos'");
	if(mysql_num_rows($sql)==1)
	{
		echo "<script>alert('POSITION TITLE ALREADY EXIST')</script>";
	}
	else
	{
		$insert=mysql_query("INSERT into position (name)VALUES(UPPER('$pos'))");
		echo "<script>alert('POSITION TITLE ADDED')</script>";
		exit();
				include("position.php");

	}
}	
?>    <h2 align="center">ADD POSITION</h2>
        <div class="container">
      <label><b>TITLE: </b></label>
      <br><br><br>
      <input type="text" autoFocus name="pos" id="pos" required>
            <label><br>
      <button id="sav" class="btn" name="sav" type="submit">SAVE</button>
